...
export class CartComponent {
  ...
  constructor() {
    const lastPromoCode = computedPrevious(this.promoCode);

    effect(() => {
      const trackPromoCode = `Promo code changes from ${lastPromoCode()} to ${this.promoCode()}`;
      console.log(trackPromoCode);
    });
  }
}